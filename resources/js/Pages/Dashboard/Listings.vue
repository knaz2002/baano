        ...
        <div class="bg-white rounded-2xl shadow-lg p-4 md:p-6">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 md:mb-6 gap-3">
                ...
                <Link 
                    ...
                    class="px-4 md:px-6 py-2 md:py-3 rounded-xl text-white font-medium text-sm md:text-base transition-all hover:shadow-lg confirm-action action-green"
            ...
            <div class="my-listings-text overflow-x-auto">
                <table class="w-full">
                    <thead>
                    ...
                    <tbody>
                        <tr v-for="listing in listings" :key="listing.id" class="border-b hover:bg-gray-50 transition-colors" style="border-color: #E8E3DA;">
                            <td class="py-3 md:py-4 px-2 md:px-4">
                                <div class="font-medium text-sm md:text-base" style="color: #1F4234;">{{ listing.title }}</div>
                            </td>
                            <td class="py-3 md:py-4 px-2 md:px-4 text-xs md:text-sm" style="color: #68736B;">{{ listing.category?.name }}</td>
                            <td class="py-3 md:py-4 px-2 md:px-4 text-xs md:text-sm font-semibold price-accent price-red" style="color: #315C47;">{{ formatPrice(listing.price) }} ₽</td>
                            <td class="py-3 md:py-4 px-2 md:px-4">
                            ...
                            <td class="py-3 md:py-4 px-2 md:px-4">
                                <div class="flex gap-1 md:gap-2">
                                    <Link :href="`/user/listings/${listing.id}/edit`" class="px-2 md:px-4 py-1 md:py-2 rounded-lg text-xs md:text-sm font-medium transition-all hover:shadow-md" style="color: #315C47; background-color: #DDE8DC;">
                                        Редактировать
                                    </Link>
            ...
            <div v-if="listings.length === 0" class="my-listings-text text-center py-8 md:py-16">
                ...
                <p class="text-base md:text-xl font-medium" style="color: #68736B;">У вас пока нет объявлений</p>
                <Link 
                    href="/user/listings/create" 
                    class="inline-block mt-4 px-4 md:px-6 py-2 md:py-3 rounded-xl text-white font-medium text-sm md:text-base transition-all hover:shadow-lg confirm-action action-green"
                    style="background-color: #315C47;"
                >
