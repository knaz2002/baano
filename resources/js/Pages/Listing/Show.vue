                        ...
                        <div class="hidden lg:flex gap-4">
                            <div class="flex flex-col gap-2 relative">
                                ...
                                <div class="flex flex-col gap-2 overflow-hidden" style="max-height: 480px;">
                                    <div v-for="(img, index) in visibleImages" :key="index" @click="currentImageIndex = thumbnailStart + index" class="w-16 h-16 rounded-lg overflow-hidden cursor-pointer border-2 transition-all flex-shrink-0" :class="currentImageIndex === thumbnailStart + index ? 'border-purple-600' : 'border-gray-200 hover:border-gray-400'">
                        ...
                        <div class="lg:hidden">
                            ...
                            <div v-if="listing.images.length > 1" class="overflow-x-auto scrollbar-hide -mx-3 px-3">
                                <div class="flex gap-2 min-w-max">
                                    <div v-for="(img, index) in listing.images" :key="index" @click="currentImageIndex = index" class="w-16 h-16 rounded-lg overflow-hidden cursor-pointer border-2 transition-all flex-shrink-0" :class="currentImageIndex === index ? 'border-purple-600' : 'border-gray-200 hover:border-gray-400'">
                                        <img :src="img" class="w-full h-full object-cover">
                                    </div>
                                </div>
                            </div>
                ...
                <div class="mt-6 md:mt-8 bg-white rounded-2xl shadow-lg">
                    <div class="border-b overflow-x-auto scrollbar-hide" style="border-color: #E7E0EC;">
                        <div class="flex gap-1 md:gap-6 px-2 md:px-6 min-w-max">
                            <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id" class="py-3 md:py-4 px-1 md:px-4 font-medium text-xs md:text-sm whitespace-nowrap transition-all border-b-2" :class="activeTab === tab.id ? 'border-purple-600 text-purple-600' : 'border-transparent text-gray-600 hover:text-gray-900'">
                                {{ tab.name }}
                            ...
                            <div v-if="similarListings.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                <Link v-for="item in similarListings" :key="item.id" :href="`/listings/${item.id}`" class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 group">
                                    ...
                                    <div class="p-5">
                                        ...
                                        <div class="mb-2">
                                            <span class="text-lg sm:text-xl font-bold" style="background: linear-gradient(135deg, #F08080 0%, #9B7FCF 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">{{ formatPrice(item.price) }} ₽</span>
                                        </div>
