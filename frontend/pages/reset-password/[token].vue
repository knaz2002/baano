...
async function onSubmit() {
  ...
  try {
    const res = await apiFetch<{ ok: boolean; message: string }>('/api/reset-password', {
      method: 'POST',
      body: {
        token: token.value,
        email: email.value,
...
<template>
  <div class="min-h-screen flex items-center justify-center bg-baano-cream px-4">
    <div class="glass p-8 rounded-2xl w-full max-w-md">
      ...
      <div
        v-else-if="errors.form || errors.email || errors.token"
        class="mb-4 p-4 bg-red-100 border border-red-400 text-red-700 rounded-md"
      >
        {{ errors.form || errors.email || errors.token }}
      </div>
      ...
      <form v-if="!success" class="space-y-4" @submit.prevent="onSubmit">
        ...
        <div>
          <label class="block text-sm font-medium mb-2 text-baano-ink">Новый пароль</label>
          <input
            v-model="password"
            type="password"
            required
            ...
            class="w-full px-4 py-2 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-baano-green"
          >
          <p v-if="errors.password" class="mt-1 text-sm text-red-600 font-semibold">
            {{ errors.password }}
          </p>
        </div>
